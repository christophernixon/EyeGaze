...
class StaticTestViewController: UIViewController {
    ...
    private var xScaling = 0.75
    private var yScaling = 0.5
...
extension StaticTestViewController: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        if testInProgress {
            ...
            self.gazePredictions.append(transformPrediction(prediction: gazePrediction))
            self.gazePredictionsCM.append(PredictionUtilities.scalePrediction(prediction: gazePrediction, xScaling: self.xScaling, yScaling: self.yScaling))
        }
//        print(self.gazePredictions)
    ...
    func transformPrediction(prediction: (Double, Double)) -> (Double, Double) {
        let (scaledX, scaledY) = PredictionUtilities.scalePrediction(prediction: prediction, xScaling: self.xScaling, yScaling: self.yScaling)
